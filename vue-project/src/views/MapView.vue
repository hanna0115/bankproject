    ...
    <div class="container">
        <div class="content">
            ...
            <div class="select-container">
                ...
                <select v-model="selectedDistrict" @change="handleDistrictChange" class="select-box"
            ...
            <div class="map-container">
                <div class="kakaomap">
                    <KakaoMap :lat="36.353705" :lng="127.341557" @onLoadKakaoMap="onLoadKakaoMap" width="100%">
                        <KakaoMapMarker v-for="(marker, index) in markerList" :key="index" :lat="marker.lat"
                            :lng="marker.lng" :infoWindow="marker.infoWindow" :clickable="true"
                            @onClickKakaoMapMarker="onClickMapMarker(marker)" />
                    </KakaoMap>
                </div>
                <div class="place-list">
                    <div class="place-item" v-for="(place, index) in placeList" :key="index"
...
const cities = [
    ...
    {
        name: '경기도',
        districts: ['고양시', '과천시', '광명시', '광주시', '구리시', '군포시', '김포시',
            '남양주시', '동두천시', '부천시', '성남시', '수원시', '시흥시', '안산시',
            '안성시', '안양시', '양주시', '오산시', '용인시', '의왕시', '의정부시',
            '이천시', '파주시', '평택시', '포천시', '하남시', '화성시', '가평군',
            '양평군', '여주시', '연천군']
    },
    {
        name: '강원도',
        districts: ['강릉시', '동해시', '삼척시', '속초시', '원주시', '춘천시', '태백시',
            '고성군', '양구군', '양양군', '영월군', '인제군', '정선군', '철원군',
            '평창군', '홍천군', '화천군', '횡성군']
    },
    {
        name: '충청북도',
        districts: ['제천시', '청주시', '충주시', '괴산군', '단양군', '보은군', '영동군',
            '옥천군', '음성군', '증평군', '진천군']
    },
    {
        name: '충청남도',
        districts: ['계룡시', '공주시', '논산시', '당진시', '보령시', '서산시', '아산시',
            '천안시', '금산군', '부여군', '서천군', '예산군', '청양군', '태안군', '홍성군']
    },
    {
        name: '전라북도',
        districts: ['군산시', '김제시', '남원시', '익산시', '전주시', '정읍시', '고창군',
            '무주군', '부안군', '순창군', '완주군', '임실군', '장수군', '진안군']
    },
    {
        name: '전라남도',
        districts: ['광양시', '나주시', '목포시', '순천시', '여수시', '강진군', '고흥군',
            '곡성군', '구례군', '담양군', '무안군', '보성군', '신안군', '영광군',
            '영암군', '완도군', '장성군', '장흥군', '진도군', '함평군', '해남군', '화순군']
    },
    {
        name: '경상북도',
        districts: ['경산시', '경주시', '구미시', '김천시', '문경시', '상주시', '안동시',
    ...
    {
        name: '경상남도',
        districts: ['거제시', '김해시', '밀양시', '사천시', '양산시', '진주시', '창원시',
...
const banks = [
    ...
    'SC제일은행', '씨티은행', 'KDB산업은행', '카카오뱅크', '토스뱅크', 'K뱅크'
];
